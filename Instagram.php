...
class Instagram {
    ...
    protected $_endpointUrls = array(
        ...
        'clientside_authorize' => 'https://api.instagram.com/oauth/authorize/?client_id=%s&redirect_uri=%s&response_type=token&scope=likes+comments+relationships',
        ...
        'tags_recent' => 'https://api.instagram.com/v1/tags/%s/media/recent?max_tag_id=%d&min_tag_id=%d&access_token=%s',
        ...
        'subscription' => 'https://api.instagram.com/v1/subscriptions/',
    );
    ...
    */
    ...
    public function addSubscription(){
	    $this->_initHttpClient($this->_endpointUrls['subscription'], CurlHttpClient::POST);
        $this->_httpClient->setPostParam('client_id', $this->_config['client_id']);
        $this->_httpClient->setPostParam('client_secret', $this->_config['client_secret']);
        $this->_httpClient->setPostParam('object', $this->_config['user']);
        $this->_httpClient->setPostParam('aspect', $this->_config['media']);
        $this->_httpClient->setPostParam('verify_token', $this->_config['verify_token']);
        $this->_httpClient->setPostParam('callback_url', $this->_config['callback_url']);
     ...
     * @return string
     */
    public function openAuthorizationUrlWithParams($params) {
    	
    	$url = 'Location: ' . $this->getAuthorizationUrl();
     ...
     * @return string
     */
    public function getClientSideAuthorizationUrl() {
        return sprintf($this->_endpointUrls['clientside_authorize'],
            $this->_config['client_id'],
            $this->_config['redirect_uri']);
    }
     ...
     * @param integer $minId
     */
     public function getRecentTag($tagName) {
        $endpointUrl = sprintf($this->_endpointUrls['tags_recent'], $tagName, '', '', $this->getAccessToken());
        $this->_initHttpClient($endpointUrl);
        $response = $this->_getHttpClientResponse();                
        return $this->parseJson($response); 
    } 
     
    public function getRecentTags($tagName, $minId) {
        $endpointUrl = sprintf($this->_endpointUrls['tags_recent'], $tagName, '', $minId, $this->getAccessToken());
        $this->_initHttpClient($endpointUrl);
        $response = $this->_getHttpClientResponse();                
        return $this->parseJson($response); 
    }

    public function getRecentTagsOldPhoto($tagName, $maxId) {
        $endpointUrl = sprintf($this->_endpointUrls['tags_recent'], $tagName, $maxId, '',$this->getAccessToken());
        $this->_initHttpClient($endpointUrl);
        $response = $this->_getHttpClientResponse();                
        return $this->parseJson($response); 
    }
